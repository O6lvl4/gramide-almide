...
`src/parser.almd`（150 KB）で、長い単語の 6 文字目に文字を打つ・消す編集 1,000 回の中央値は 10 µs、
90 パーセンタイルは 14 µs で、丸ごとのパースは 4.3 ms。50 回に 1 回は丸ごとのパースと照合しています
（[証拠](docs/evidence/incremental-parser-almd.json)、`bench/incremental.py`）。
