...
fixtures: `check` and `outline` on valid and broken files, `version`,
`languages`, and structured-range cases (blocks, heredocs, a multi-byte
character above a name, invalid input). No model API or credentials are used.
...
whole-corpus measurement in the README is reproduced with gramide's
`bench/corpus_check.py --binary ./gramide_almide --root <almide checkout> --ext .almd`.
