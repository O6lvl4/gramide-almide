...
"$compiler" test
"$compiler" build cli/main.almd --release -o gramide_almide
./gramide_almide gen-table | diff -u src/table.almd - \
  || { echo "src/table.almd is not what the grammar compiles to: ./gramide_almide gen-table > src/table.almd"; exit 1; }
python3 ci/smoke.py
