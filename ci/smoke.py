...
with tempfile.TemporaryDirectory() as tmp:
    root = Path(tmp)
    source = root / "valid.almd"
    source.write_text('fn real() -> Int = 1\n')
    run("check", source)
    assert "function real" in run("outline", source)
    broken = root / "broken.almd"
    broken.write_text('fn real() -> Int = 1\n}\n')
    run("check", broken, code=1)
    assert run("version").splitlines()[0].startswith("gramide_almide ")
    assert '"id":"almide"' in run("languages")
